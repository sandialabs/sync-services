...
def listify(expression):
    ...
    expression = re.sub(r"([( ])([a-zA-Z0-9-_*]+)([ )])", r'\1"\2"\3', expression)
    ...
    if expression.startswith("(directory "):
        return (
            ...
            "<ul>{}{}</ul>".format(
                ...
                "".join(
                    '<li><a href="{}/">{}</a></li>'.format(urllib.parse.quote(x), x)
...
def handle_peer():
    if request.method == "GET":
        ...
        result = query(
            '(ledger-peer! {name} (lambda (m) (sync-remote "{url}" m)))'.format(
...
if __name__ == "__main__":
    ...
    app.config["JOURNAL"] = args.journal if args.journal else os.getenv("JOURNAL")
