...
add_user() {
    ...
    if ! id "$username" &>/dev/null; then
        [[ "$username" != "$SAMBA_USERNAME" ]] && echo "User $username does not exist, creating user..."
        extra_args=()
    ...
    if echo "$pdb_output" | grep -q "^$username:"; then
        ...
        [[ "$username" != "$SAMBA_USERNAME" ]] && echo "User $username has been added to Samba and password set."
    fi
...
if [ -f "$config" ] && [ -s "$config" ]; then
    ...
    if [ -n "$SAMBA_SHARE" ] && [[ "${SAMBA_SHARE,,}" != "data" ]]; then
      sed -i "s/\[Data\]/\[$SAMBA_SHARE\]/" "$config"    
    fi

    # Update force user and force group in smb.conf
    sed -i "s/^\(\s*\)force user =.*/\1force user = $SAMBA_USERNAME/" "$config"
...
if [ -f "$users" ] && [ -s "$users" ]; then
    ...
    add_user "$config" "$SAMBA_USERNAME" "$UID" "$group" "$GID" "$SAMBA_SECRET" "$share" || { echo "Failed to add user $SAMBA_USERNAME"; exit 1; }
    ...
    if [[ "$RW" != [Ff0]* ]]; then
        ...
        if [ -z "$(ls -A "$share")" ]; then
            ...
            chown "$SAMBA_USERNAME:$group" "$share" || { echo "Failed to set ownership for directory $share"; exit 1; }
        fi
    fi
...
python service.py --back /srv/data-back --front /srv/data-front --journal "http://journal.docker/interface" --secret $SECRET &
